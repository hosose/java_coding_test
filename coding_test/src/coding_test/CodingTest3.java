...
public class CodingTest3 {
	...
	public static int solution(int[] schedules, int[][] timelogs, int startday) {
		...
		for (int i = 0; i < timelogs.length; i++) {
			int personalSchedule = schedules[i];
			int[] personalTimeLogs = timelogs[i];
			int goodJob = 0;
			int personalTLlength = 0;
			int personalStartDay = startday-1;

			int deadlineTime = formatTime(personalSchedule, "next");

			for (int j = 0; j < personalTimeLogs.length; j++) {
				personalStartDay++;
				if (personalStartDay % 7 == 6 || personalStartDay % 7 == 0) {
					continue;
				}
				personalTLlength++;

				if (personalTimeLogs[j] <= deadlineTime) {
					goodJob++;
				}else {
					break;
				}
			}
			if (goodJob == personalTLlength) {
				answer++;
			}
		}
		return answer;
	...
	public static int formatTime(int time, String div) {
		int hours = time / 100; // 7
		int minutes = time % 100; // 5

		// 총 분으로 변환하여 10분 빼기
		int totalMinutes = hours * 60 + minutes; // 7 * 60 + 5 = 425분

		if ("prev".equals(div)) {
			totalMinutes -= 10; // 425 - 10 = 415분
		} else {
			totalMinutes += 10; // 425 - 10 = 415분
		}

		// 다시 시와 분으로 변환
		int newHours = totalMinutes / 60; // 415 / 60 = 6시간
		int newMinutes = totalMinutes % 60; // 415 % 60 = 55분

		// 자정을 넘으면 24시를 00시로 바꿔야 합니다.
		if (newHours >= 24) {
			newHours -= 24; // 24 -> 0, 25 -> 1...
		} else if (newHours < 0) {
			newHours += 24;
		}

		int timeAsInt = newHours * 100 + newMinutes;
		return timeAsInt;
	}
}
